...
def getModelofFeatures(signal,Fs):
    ...
    for i in range(noOfFeatures):
        modelFS = numpy.zeros((1, 4))
        modelFS[:,0] = numpy.mean(stFeatures[i,:])
        modelFS[:,1] = numpy.std(stFeatures[i,:])
        modelFS[:,2] = numpy.max(stFeatures[i,:])
        modelFS[:,3] = numpy.min(stFeatures[i,:])
        finalFeatures.append(modelFS)
    finalFeatures = numpy.concatenate(finalFeatures, 1)
    return finalFeatures


def writeSingleFileToCSV(filePath,pathToCSV,species):
    
    [Fs, signal] = readSegment(filePath)
    finalFeatures = getModelofFeatures(signal,Fs)
    finalFeatures = finalFeatures.tolist()
    finalFeatures[0].append( species )
    with open(pathToCSV, "ab") as f:   
        writer = csv.writer(f,delimiter=',')    
        writer.writerows(finalFeatures)
    

def featuresToCSV(pathToSegments,pathToCSV,species):
    for root, dirs, files in os.walk(pathToSegments):
        for file in files:
            if file.endswith(".wav"):
                filePath = os.path.join(root, file)
                print("processing segment " + filePath)
                writeSingleFileToCSV(filePath,pathToCSV,species)
        break 
